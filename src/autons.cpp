...
void red_right_side() {
    ...
    chassis.turnToPoint(-23.5, -23.5, TO);
    waitd;
...
// WRONG PATH BC DN IF WE CAN SCORE ON WALL STAKE!
void solo_awp() {
    chassis.setPose(-56.683, 19.75, 180);

    chassis.moveToPoint(-56.683, 0, TO);
    waitd;

    chassis.turnToHeading(270, TO, {.maxSpeed=75});
    waitd;
    chassis.moveToPoint(-65.85, chassis.getPose().y, TO);
    waitd;

    intake.intake();

    // chassis.turnToHeading(50, TO);
    // pros::delay(250);
    // arm.move(2700);
    // waitd;

    // chassis.moveToPoint(-59.325, 9.233, TO, {false});
    // waitd;

    // pros::delay(100);
    // arm.move(-275);

    // chassis.moveToPoint(-54.73, 15.022, TO);
    // waitd;

    // intake.toggle();

    // chassis.turnToPoint(-52.204, -0.291, TO, {.forwards=false});
    // waitd;
    // chassis.moveToPoint(-52.204, -0.291, 2000, {.forwards=false, .maxSpeed=50});
    // intake.intake();
    // waitd;
    // intake.toggle();
    // pros::delay(750);

    // chassis.moveToPoint(-54.211, 19.167, 3000, {.maxSpeed=35});
    // waitd;
    // pros::delay(500);
    // intake.brake();

    
    // chassis.turnToPoint(-23.846, 20.472, TO);
    // waitd;
    // chassis.moveToPoint(-23.846, 20.472, TO, {.maxSpeed=80});
    // chassis.waitUntil(20);
    // mogo.toggle();
    // waitd;


    // chassis.turnToPoint(-13.567,32.766, TO, {false});
    // waitd;
    // intake.intake();
    // chassis.moveToPoint(-13.567,32.766, TO, {.forwards=false, .maxSpeed=60});
    // // chassis.moveToPoint(-27.22, 16.579, TO);
...
void blue_right_side() {    
    ...
    turnAndMoveToPoint(21.25, 22.25, 1000, 1000);
    // chassis.turnToPoint(21.25, 22.5, 750);
    ...
    // THIS
    // turnAndMoveToPoint(12, 52, 500, 2000, false);
    ...
    turnAndMoveToPoint(23.5, 47.75, 1000, 1000, false);
    // waits a bit to intake
    pros::delay(1000);

    turnAndMoveToPoint(9, 12, 1000, 1500, false);
