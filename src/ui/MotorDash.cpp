...
MyDevice::MyDevice(
    ...
    , std::int8_t* motor_port
    , std::uint8_t* pneumatic_port
) {
    name = device_name;
    ...
    switch (info.type) {
        case DeviceType::MOTOR:
            info.motor = new pros::Motor(*motor_port);
            break;
        case DeviceType::PNEUMATIC:
            info.pneumatics = new pros::adi::Port(*pneumatic_port);
            break;
    }
}

// std::vector<MyDevice> devices = {
//     {"FRONT (L)", DeviceType::MOTOR, &LEFT_FRONT_PORT, nullptr}
//     // , {"FRONT (M)", LEFT_MID_PORT}
//     // , {"FRONT (B)", LEFT_BACK_PORT}
